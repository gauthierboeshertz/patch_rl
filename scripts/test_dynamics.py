...
def compute_mask_metrics(pred_masks_prob,gt_masks,threshold=0.2):
    ...
    pred_masks_prob = pred_masks_prob.flatten()
    gt_masks_flat = gt_masks.flatten()
    
    pred_masks_flat = (pred_masks_prob > threshold).astype(int)
    ...
    print(all_metrics)
    roc_curve = RocCurveDisplay.from_predictions(gt_masks_flat, pred_masks_prob)
...
def get_masks_probs_and_gt(obs,action,next_obs,dynamics ,encoder_decoder):
    ...
    gt_causal_mask = make_gt_causal_mask(images.cpu(),action.cpu(),patch_size=encoder_decoder.patch_size,num_sprites=dynamics.action_embedding.num_actions)
    pred_causal_masks_probs, _ = dynamics.get_causal_mask(t_obs/255., t_action,encoder=encoder_decoder,head_fusion="max",discard_ratio=0.98)
