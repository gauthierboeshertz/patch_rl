...
class MLPPatchVAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc_mu = nn.Linear(channels[-1], embed_dim)
        self.fc_var = nn.Linear(channels[-1], embed_dim)

        channels.reverse()
        self.decoder = MLP(self.embed_dim, input_dim,layer_sizes=channels[:-1],activation=nn.LeakyReLU,norm_type=None)
    ...
    def compute_loss_encodings(self, obs):
        ...
        patches = self.images_to_patches(tobs) # einops.rearrange(tobs, "b c (h p1) (w p2) -> (b h w) c p1 p2", p1=self.patch_size, p2=self.patch_size)
