...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.,use_rel_pos=-1):
        ...
        if use_rel_pos > 0:
            self.rel_pos = nn.Parameter(torch.randn(use_rel_pos,use_rel_pos,inner_dim))
...
class Transformer(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        for attn, ff in self.layers:
            attn_x, attn_weight = attn(x,attn_mask=attn_mask)
        ...
        return x, torch.stack(attn_weights,axis=1)
